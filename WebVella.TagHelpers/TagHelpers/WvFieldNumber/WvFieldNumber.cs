...
{
	[HtmlTargetElement("wv-field-number")]
	[RestrictChildren("wv-field-prepend", "wv-field-append")]
	public class WvFieldNumber : WvFieldBase
	{
		...
		public override async Task ProcessAsync(TagHelperContext context, TagHelperOutput output)
		{
			...
			else if (Mode == WvFieldRenderMode.Display)
			{
                ...
                if (Value != null)
				{
					...
					divEl.AddCssClass("form-control-plaintext erp-number");
					var valueString = Value.ToString("N" + DecimalDigits, Culture);
					divEl.InnerHtml.Append(valueString);
			...
			else if (Mode == WvFieldRenderMode.InlineEdit)
			{
				if (Access == WvFieldAccess.Full || Access == WvFieldAccess.FullAndCreate)
				{
					#region << View Wrapper >>
					{
						...
						viewFormControlEl.AddCssClass("form-control erp-number");
						var valueString = "";
						if (Value != null)
						{
							valueString = Value.ToString("N" + DecimalDigits, Culture);
						}
						viewFormControlEl.InnerHtml.Append(valueString);
					...
					#region << Edit Wrapper>>
					{
						...
						editInputEl.AddCssClass("form-control erp-number");
					...
					{
						var tagHelperContext = (WvTagHelperContext)ViewContext.HttpContext.Items[typeof(WvFieldNumber) + "-inline-edit"];
					...
					var scriptTemplate = @"
						$(function(){
							NumberInlineEditInit(""{{FieldId}}"",""{{Name}}"",{{ConfigJson}});
						});";
					...
					var fieldConfig = new WvFieldNumberConfig()
					{
						ApiUrl = ApiUrl,
				...
				else if (Access == WvFieldAccess.ReadOnly)
				{
					...
					//Control
					var inputEl = new TagBuilder("input");
					inputEl.AddCssClass("form-control erp-number");
					...
					inputEl.Attributes.Add("value", valueString);
					inputEl.Attributes.Add("readonly", null);
